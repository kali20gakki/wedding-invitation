...
| C15 | 风车邮局来信换成真实婚纱照（原图风格，不转画风） | ✅ **已完成**（10 张全量，见下） |
| C16 | 删末张留 9 张 / 图片高度按可用空间算 / 图注去掉方块底 | ✅ **已完成** |

---

## C15 · 风车邮局来信：换成真实婚纱照（10 张全量）

### 需求

新人要求把 03 章的立绘换成**真实婚纱照**，并且**明确不要转换风格、要用原图**；
后续又明确"**文件夹下的图片都要展示**，不是只展示三张"，并要求**去掉诗经引文**、只留白话。

### 素材与产出

`婚礼照片/` 共 10 张原图（单张 9–29MB、合计约 170MB），**全部采用**。
`tools/build-wedding-photos.py` **只做等比缩小 + 转 WebP，不做任何风格处理**。

每种尺寸出两档，配合 `srcset` 让手机只下载小的那一档：

| 档位 | 长边 | 用途 | 单张上限 |
| --- | --- | --- | --- |
| `photo-NN.webp` | 1280 | 桌面 / 高分辨率屏 | 95 KB |
| `photo-NN@640.webp` | 640 | 手机（卡片实际显示约 308px，@2x 即 616px） | 45 KB |

`assets/crew/` 共 20 个文件、**981 KB**。手机上实际只加载 `@640` 那一档，约 **276 KB**。
实测 `currentSrc` 在 390px 视口下解析为 `photo-01@640.webp`，确认 `srcset` 生效。

HTML 里第一张不加 `loading="lazy"`（进入视口前就该就绪），其余 9 张加 `lazy`，
避免首屏一次拉完 10 张。

### 编号 → 画面内容（逐张看图确认，写进脚本注释）
...
轮播顺序 = `photo-01..10`，打散场景类型以免黑白棚拍连在一起。

### ⚠️ 踩坑：按"排序序号"选片连错两次

第一版用 `sorted(files)[7]` 定位源图，**连错两次**：

- `sorted` 后第 8 张是 `_4971_`，不是 `_4968_`
- 第 6 张是 `_4969_`，不是 `_4972_`

原因：文件名的**时间戳前缀**和**照片编号**顺序并不一一对应，
按序号写等于把"我看图时数的第几张"和"程序排序后的第几张"当成一回事。

修法：**改用照片自身的编号定位**（`f'_{serial}_' in path.name`），
并把"编号 → 画面内容"的对应表写进脚本注释。
**结论：凡是按内容选素材，定位键必须稳定且语义明确，不能用排序位置。**

### 画幅：4:3 横版 → 竖版

婚纱照多为 2:3 竖版，而卡片原来是 `aspect-ratio:4/3` 横版 —— 会**把竖版裁掉大半**。
改成 `height:clamp(300px,52svh,420px)` + `object-position:center 32%`，
实测图片区 308×420，10 张逐张截图确认人物没有被裁切。
圆角与裁切也从 `.crew-slide` 移到 `img` 上 —— 原来挂在容器上会把下面的图注一起裁掉。

### 文案：去掉诗经引文，只留白话

初版是"古句起头 + 白话承接"（亲结其缡／风雨如晦／桃之夭夭），
按要求**去掉引文**，只保留贴合画面的白话一句，10 张各一条：

> 烛火一支支亮起来的时候，我才发觉，原来等待也可以这么温柔。
> 白纱把周围都变得很安静，只剩心跳，一下一下，数着日子。
> 你站在光里等我，捧花记得那一刻的所有颜色。
> 你替我掀开这层薄纱，从此所有郑重的事，都只想与一个人一同完成。
> 裙摆铺开像一整片花海，每一朵都在说同一句祝福。
> 那把透明的伞遮得住雨，也容得下相视一笑；往后的每一场雨，都不必一个人走。
> 草地很大，可我只看见你伸过来的那只手。
> 光从拱窗斜进来落在白纱上。要出发了，门外有风、有远山，也有等了很久的你。
> 我们隔着一点距离站着，却早就站在了同一条路上。
> 红幕与圆月之间，我们穿着最郑重的礼服，慢慢走向彼此。

同时删掉了已无用的 `.crew-slide p i` 样式，并把计数器初值从写死的 `1 / 3` 改成 `1 / 10`。

### 入库处理

`婚礼照片/` 已加入 `.gitignore`（170MB 原图不进仓库，与 `raw-photos/`、`抽卡图片/` 同样处理）。
web 成品在 `assets/crew/`。

---

## C16 · 风车邮局来信：删末张、收图片高度、去掉图注方块底

### 三处改动

1. **删掉最后一张**（红幕圆月），轮播由 10 张变 9 张，计数器改 `1 / 9`，
   同时删除已无用的 `assets/crew/photo-10*.webp`。
2. **图片高度改为按可用空间算**，不再用 `52svh`。
3. **图注去掉方块底色** —— 它原来挂着 `background:var(--linen)` + 直角，
   压在圆角图片下面就是一块直角色块，与图片圆角明显割裂。
   现在改成透明底、与图片同圆角（图片 12px）、下方留白。

### ⚠️ 关键教训：同一选择器留了两份规则，改了半天不生效

排查图片高度时，改了三次 `height` 都**没有任何变化**（computed 始终是旧值）。
原因是文件里**同时存在两条 `.crew-slide img` 规则**：
后定义的那条把先定义的整个盖住，而我一直在改前一条。

同理还有 `.crew-slide p` 也留了两条（一条是透明底、一条是 `--linen` 方块底）——
"图注有方块阴影"的根因就是这个：**后一条旧规则一直在生效**。

> 规则：**改样式前先 grep 一遍选择器，确认只有一条。**
> 多版本叠加是隐性 bug，比写错值更难发现 —— 因为它不报错，只是"不起作用"。

### 另一个真凶：遗留的 padding 覆盖块

`style.css` 末尾的「整屏节奏微调」里还留着三条旧覆盖：

```css
.crew-section .crew-phone{padding:18px}
.crew-section .crew-slide p{padding:40px 4px 28px}     /* +68px */
.crew-section .crew-controls{padding:30px 4px 2px}     /* +32px */
```

这是图注还很短、翻页控件还小的时候加的。换成大图 + 长文案之后，
它们白白多占约 **118px** 竖向空间，直接导致矮屏上图片必须缩到 170px 才放得下一屏。
删掉后同样的屏高下图片能大一档。

> 教训：**改了组件结构，要回头清掉为旧结构加的覆盖值。**
> 这类"历史补丁"散落在文件末尾的微调区，最容易被忘掉。

### 图片高度公式

本章除图片外的固定开销 = 内容区上下内边距(64+36) + 标题 54 + 相框边框/内边距
+ 顶部条 + 图注 + 翻页控件 ≈ **420px**，所以：

```css
height:clamp(150px, calc(100svh - 420px), 400px)
```
...
| 视口 | 屏数 | 图片 |
| --- | --- | --- |
| 360×640 | 1.000 | 286×220 |
| 375×667 | 1.000 | 301×247 |
| 390×844 | 1.000 | 316×400 |
| 390×962 | 1.000 | 316×400 |
| 430×932 | 1.000 | 356×400 |

（此前是 1.087 / 1.084 / 1.026 —— 矮屏与 844 都超出了。）

### 顺带发现

`tools/shot.mjs` 与 `shoot.mjs` 截的页面会被 Chrome 缓存，
改了 CSS 后必须加 `?v=<时间戳>` 破缓存，否则量到的是旧样式 —— 这会让人误判"改动没生效"。



---

## C15 · 风车邮局来信：换成真实婚纱照（保留原图风格）

### 需求

新人要求把 03 章的三张**立绘换成真实婚纱照**，并且**明确不要转换风格、要用原图**。
（此前 01/03 章的人像都是 Ghibli 风格立绘，这一章改为写实照片。）

### 素材

`婚礼照片/` 共 10 张原图，单张 9–29MB、合计约 170MB。全部逐张看过图后选了 3 张，
构成"由外到内、由动到静"的节奏：

| 目标文件 | 照片编号 | 画面 |
| --- | --- | --- |
| `couple-umbrella.webp` | 4969 | 林间各撑一把透明伞相望 |
| `couple-veil.webp` | 4971 | 古门前共读，新郎为新娘掀纱（烛台） |
| `couple-window.webp` | 4973 | 拱窗前的新娘剪影，满地花瓣 |

`tools/build-wedding-photos.py`：**只做等比缩小 + 转 WebP，不做任何风格处理**
（不调色、不换画风），长边 1440px、q82，三张合计 **373 KB**。

### ⚠️ 踩坑：按"排序序号"选片连错两次

第一版用 `sorted(files)[7]` 这种方式定位源图，结果**连错两次**：

- `sorted` 之后第 8 张是 `_4971_`，不是 `_4968_`
- 第 6 张是 `_4969_`，不是 `_4972_`

原因：文件名的**时间戳前缀**和**照片编号**并不一一对应顺序，
按序号写等于把"我看图时数的第几张"和"程序排序后的第几张"当成一回事了 —— 而它们不一致。

修法：**改用照片自身的编号定位**（文件名里的 `_4969_` 那一段）：
...
并把"编号 → 画面内容"的对应关系连同**逐张看图确认**的结果一起写进脚本注释，
下次改选片不用再猜。**结论：凡是按内容选素材，定位键必须是稳定且语义明确的，
不能用排序位置。**
...
```css
.crew-slide img{height:clamp(300px,52svh,420px);object-fit:cover;object-position:center 32%}
```

实测图片区域 308×420，人物完整在框内（逐张截图确认过脸没有被切）。
圆角与裁切也从 `.crew-slide` 移到了 `img` 上 —— 原来挂在容器上会把下面的图注一起裁掉。

### 文案：古句起头 + 白话落点

每张配一句古语作引，后接一句贴合画面内容的白话：

| 照片 | 引 | 承接 |
| --- | --- | --- |
| 烛前掀纱 | *亲结其缡，九十其仪。*（《诗经·东山》） | 烛火摇着，你为我掀开这层薄纱——从此所有郑重的事，都只想与一个人一同完成。 |
| 林间撑伞 | *风雨如晦，鸡鸣不已。*（《诗经·风雨》） | 那把透明的伞，遮得住雨，也容得下相视一笑——好在往后的每一场雨，都不必一个人走。 |
| 拱窗剪影 | *桃之夭夭，灼灼其华。*（《诗经·桃夭》） | 光从拱窗斜进来，落在白纱上。要出发了，门外有风、有远山，也有等了很久的你。 |

古句用 `.crew-slide p i` 排成独立一行、衬线加粗，与白话正文区分开。

### 入库处理

`婚礼照片/` 已加入 `.gitignore`（170MB 原图不进仓库，与 `raw-photos/`、`抽卡图片/` 同样处理）。
web 用的成品在 `assets/`，三张合计 373 KB。
